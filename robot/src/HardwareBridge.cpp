...
void MiniCheetahHardwareBridge::run() {
  ...
  // rc controller
  _port = init_sbus(false);  // Not Simulation 遥控器rc定时器周期5ms，200hz
...
#ifdef USE_TCPnet_Control_WithOut_AT9S
...
//        printf("lord quad:%.5f\t%.5f\t%.5f\t%.5f\n",_microstrainImu.quat[0],_microstrainImu.quat[1],_microstrainImu.quat[2],
//               _microstrainImu.quat[3]);
//        printf("rate:%.4f\t%.5f\t%.4f\n",_microstrainImu.gyro[0],_microstrainImu.gyro[1],_microstrainImu.gyro[2]);
//        printf("acc:%.2f\t%.2f\t%.2f\n",_microstrainImu.acc[0],_microstrainImu.acc[1],_microstrainImu.acc[2]);
//        printf("---------------------------------\n");
