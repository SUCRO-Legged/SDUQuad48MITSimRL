...
//tmp
spine_data_t g_spine_data_front;
spine_data_t g_spine_data_hind;
...
const float wimp_torque[3] = {19.f, 19.f, 19.f};    // TODO CHECK WITH BEN
...
//const float hip_offset[4] = {(M_PI-3/180*M_PI) / 2.f, -(M_PI-3/180*M_PI) / 2.f, (M_PI-3/180*M_PI) / 2.f, -(M_PI-3/180*M_PI) / 2.f};
//const float hip_offset[4] = {-1.15f, 1.15f, -1.15f, 1.15f};
const float hip_offset[4] = {-1.16f, 1.16f, -1.16f, 1.16f};
//const float hip_offset[4] = {-1.203f, 1.203f, -1.203f, 1.203f};
...
//        {
//            g_spine_data_front.q_abad[iii]= spine_data->q_abad[iii];
//            g_spine_data_front.q_hip[iii]= spine_data->q_hip[iii];
//            g_spine_data_front.q_knee[iii]= spine_data->q_knee[iii];
//        }
//    }
//    else
//    {
//        for(int iii=0;iii<2;iii++)
//        {
//            g_spine_data_hind.q_abad[iii]= spine_data->q_abad[iii];
//            g_spine_data_hind.q_hip[iii]= spine_data->q_hip[iii];
//            g_spine_data_hind.q_knee[iii]= spine_data->q_knee[iii];
//        }
//    }
//    static int show_raw_data_times=0;
//    show_raw_data_times++;
//    if(show_raw_data_times%500==0)
//        printf("RH_hip_q : %.2f\n",g_spine_data_hind.q_hip[1]);
...
void  spi_send_receive(spi_command_t *command, spi_data_t *data) {
  ...
  for (int spi_board = 0; spi_board < 2; spi_board++) {
    ...
    // copy into tx buffer flipping bytes 转换成16位到数据格式
...
//    for (int i = 0; i < 30; i++)
      for (int i = 0; i < 42; i++) // 加上扭矩反馈后变成42个 16-bit数据
