...
Simulation::Simulation(RobotType robot, Graphics3D* window,
  ...
  if (_window) {
    ...
    truthColor << 0.1, 0.1, 0.1, 0.6; // 近似黑色，实际的机器人
    seColor << .75,.75,.75, 1.0;      // 近似白色                                     //颜色   // 使用old颜色 // 可隐藏
  ...
  _simulator = new DynamicsSimulator<double>(_model, (bool)_simParams.use_spring_damper); //根据参数指定是否使用弹簧阻尼
  _robotDataSimulator = new DynamicsSimulator<double>(_robotDataModel, false); // 不使用弹簧阻尼
...
// 仿真中到底层控制器,用仿真数据当作实际机器人上到spie板采集的数据
void Simulation::lowLevelControl() {
  if (_robot == RobotType::MINI_CHEETAH) {
    ...
    // run spine board control: // 模拟运行spi板子计算关节扭矩功能
...
void Simulation::highLevelControl() {
  ...
  _sharedMemory().simToRobot.gamepadCommand.applyDeadband(
      _simParams.game_controller_deadband); // 施加死区

  // send IMU data to robot: 仿真的机器人状态更新为imu采集到的数据
  ...
  _robotMutex.lock();
  _sharedMemory().simToRobot.mode = SimulatorMode::RUN_CONTROLLER; // 通知机器人可以运行控制器
  _sharedMemory().simulatorIsDone(); // 仿真端的工作完成
