...
void LegController<T>::updateData(const SpiData* spiData) {
  for (int leg = 0; leg < 4; leg++) {
      if(std::isnan(spiData->q_abad[leg]))
      {
          ...
          // qd
          datas[leg].qd(0) = spiData->qd_abad[leg];//*1.25;
          datas[leg].qd(1) = spiData->qd_hip[leg];//*1.25;
          datas[leg].qd(2) = spiData->qd_knee[leg];//*1.25;
      }

/*
//    datas[leg].qd(0) = spiData->qd_abad[leg]/2.0;
//    datas[leg].qd(1) = spiData->qd_hip[leg]/2.0;
//    datas[leg].qd(2) = spiData->qd_knee[leg]/2.0;

    //用数值计算来替换电机传递来到速度,   效果不好
//    datas[leg].qd_numeric = datas[leg].qd_numeric_buffer*0.5+0.5*(datas[leg].q - datas[leg].q_buffer) / (0.002); //rad/s
//    datas[leg].q_buffer= datas[leg].q;
//    datas[leg].qd_numeric_buffer = datas[leg].qd_numeric;
//    datas[leg].qd =  datas[leg].qd_numeric;


//    if(leg==0)
//    {
//        printf("qd: %.3f\t%.3f\n",datas[leg].qd(2),datas[leg].qd_numeric(2) );
//      //  std::cout<<"datas[leg].qd_buffer(2)"<<datas[leg].qd(2)<<"\t"<<datas[leg].qd_numeric(2)<<std::endl;
//        static std::ofstream log("/home/user/log/log_joint_v.csv");
//        log<< datas[leg].qd(2)<<",";
//        log<< datas[leg].qd_numeric(2)<<",";
//        log<<",";
//        log<<std::endl;
//
//
//    }
      //速度滤波,发现运动不稳定
//      datas[leg].qd(0) = datas[leg].qd_buffer(0)*0.5 + 0.5*spiData->qd_abad[leg];
//      datas[leg].qd(1) = datas[leg].qd_buffer(1)*0.5 + 0.5*spiData->qd_hip[leg];
//      datas[leg].qd(2) = datas[leg].qd_buffer(2)*0.5 + 0.5*spiData->qd_knee[leg];
//      datas[leg].qd_buffer=datas[leg].qd;

    //10次缓冲计算平均速度, 运动更加不稳定,站立不动时关节速度波动下降到+-0.1rad/s
//      for(int i=9;i>0;i--)
//          datas[leg].qd_buffer_10[i]=datas[leg].qd_buffer_10[i-1];
//      datas[leg].qd_buffer_10[0](0) = spiData->qd_abad[leg];
//      datas[leg].qd_buffer_10[0](1) = spiData->qd_hip[leg];
//      datas[leg].qd_buffer_10[0](2) = spiData->qd_knee[leg];
//      double qd0_sum =0,qd1_sum=0,qd2_sum=0;
//      for(int i=9;i>=0;i--) {
//          qd0_sum += datas[leg].qd_buffer_10[i](0);
//          qd1_sum += datas[leg].qd_buffer_10[i](1);
//          qd2_sum += datas[leg].qd_buffer_10[i](2);
//      }
//      datas[leg].qd(0)=qd0_sum/10.0;
//      datas[leg].qd(1)=qd1_sum/10.0;
//      datas[leg].qd(2)=qd2_sum/10.0;
*/
...
//  {
//      if(leg == 3) {
//          printf("fxyz : %.2f\t%.2f\t%.2f\t\t", datas[leg].footforceActuatual(0), datas[leg].footforceActuatual(1),
//                 datas[leg].footforceActuatual(2));
//          printf("Desired : %.2f\t%.2f\t%.2f\t\t\n", datas[leg].footforceDesired(0), datas[leg].footforceDesired(1),
//                 datas[leg].footforceDesired(2));
//      }
//  }
...
//        if(fabs(datas[leg].tauEstimate[mx])>48.0)
//            datas[leg].tauEstimate[mx]=datas[leg].tauEstimate[mx]/fabs(datas[leg].tauEstimate[mx])*48.0;

      spiCommand->flags[leg] = _legsEnabled ? 1 : 0;
  }
 // printf("tau f%.2f\t%.2f\t%.2f\t\t",datas[0].tauEstimate[0],datas[0].tauEstimate[1],datas[0].tauEstimate[2]);
 // printf("tau b%.2f\t%.2f\t%.2f\n",datas[2].tauEstimate[0],datas[2].tauEstimate[1],datas[2].tauEstimate[2]);
}
...
//            lcmData->tau_est[idx] = datas[leg].tauEstimate[axis];
            lcmData->tau_est[idx] = datas[leg].tauActuatual[axis]; // 实际扭矩
...
void computeLegJacobianAndPosition(Quadruped<T>& quad, Vec3<T>& q, Mat3<T>* J,
  ...
  if(std::isnan(p->operator()(0)))
  {
    printf("isnan: leg update %.2f\t%.2f\t%.2f\n",s1,s2,s3);
  }
}
