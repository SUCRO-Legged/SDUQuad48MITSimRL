...
class ControlParameter {
 public:
  ...
  ControlParameter(const std::string& name, Vec3<double>& value,
    ...
    /*!
   * Construct control parameter for a list of 6 floats
   * @param name : name of parameter
   * @param value : reference to value
   * @param collection : collection to add to
   * @param units : name of units
   */
    ControlParameter(const std::string& name, Vec6<float>& value,
                     ControlParameterCollection& collection,
                     const std::string& units = "") {
        _name = name;
        truncateName();
        _units = units;
        _value.vec6f = value.data();
        _kind = ControlParameterValueKind::VEC6_FLOAT;
        collection.addParameter(this, name);
    }
    /*!
     * Construct control parameter for a list of 6 doubles
     * @param name : name of parameter
     * @param value : reference to value
     * @param collection : collection to add to
     * @param units : name of units
     */
    ControlParameter(const std::string& name, Vec6<double>& value,
                     ControlParameterCollection& collection,
                     const std::string& units = "") {
        _name = name;
        truncateName();
        _units = units;
        _value.vec6d = value.data();
        _kind = ControlParameterValueKind::VEC6_DOUBLE;
        collection.addParameter(this, name);
    }
  /*!
  ...
  void initializeVec3d(const Vec3<double>& v) {
     ...
     * Checks to see that the types are correct
     */
    void initializeVec6d(const Vec6<double>& v) {
        if (_kind != ControlParameterValueKind::VEC6_DOUBLE) {
            throw std::runtime_error("Tried to initialize control parameter " +
                                     _name + " as a vector6d");
        }
        _set = true;
    ...
    void initializeVec6f(const Vec6<float>& v) {
        if (_kind != ControlParameterValueKind::VEC6_FLOAT) {
            throw std::runtime_error("Tried to initialize control parameter " +
                                     _name + " as a vector6d");
        }
        _set = true;
  ...
  void set(ControlParameterValue value, ControlParameterValueKind kind) {
    ...
    switch (kind) {
      ...
      case ControlParameterValueKind::VEC3_DOUBLE:
        ...
        break;
        case ControlParameterValueKind::VEC6_FLOAT:
            _value.vec6f[0] = value.vec6f[0];
            _value.vec6f[1] = value.vec6f[1];
            _value.vec6f[2] = value.vec6f[2];
            _value.vec6f[3] = value.vec6f[3];
            _value.vec6f[4] = value.vec6f[4];
            _value.vec6f[5] = value.vec6f[5];
            break;
        case ControlParameterValueKind::VEC6_DOUBLE:
            _value.vec6d[0] = value.vec6d[0];
            _value.vec6d[1] = value.vec6d[1];
            _value.vec6d[2] = value.vec6d[2];
            _value.vec6d[3] = value.vec6d[3];
            _value.vec6d[4] = value.vec6d[4];
            _value.vec6d[5] = value.vec6d[5];
            break;
      default:
  ...
  ControlParameterValue get(ControlParameterValueKind kind) {
    ...
    switch (_kind) {
      ...
      case ControlParameterValueKind::VEC3_DOUBLE:
        ...
        break;
      case ControlParameterValueKind::VEC6_FLOAT:
            value.vec6f[0] = _value.vec6f[0];
            value.vec6f[1] = _value.vec6f[1];
            value.vec6f[2] = _value.vec6f[2];
            value.vec6f[3] = _value.vec6f[3];
            value.vec6f[4] = _value.vec6f[4];
            value.vec6f[5] = _value.vec6f[5];
            break;
      case ControlParameterValueKind::VEC6_DOUBLE:
            value.vec6d[0] = _value.vec6d[0];
            value.vec6d[1] = _value.vec6d[1];
            value.vec6d[2] = _value.vec6d[2];
            value.vec6d[3] = _value.vec6d[3];
            value.vec6d[4] = _value.vec6d[4];
            value.vec6d[5] = _value.vec6d[5];
            break;
      default:
  ...
  std::string toString() {
    ...
    switch (_kind) {
      ...
      case ControlParameterValueKind::VEC6_FLOAT:
            result += "[";
            result += numberToString(_value.vec6f[0]) + ", ";
            result += numberToString(_value.vec6f[1]) + ", ";
            result += numberToString(_value.vec6f[2]) + ", ";
            result += numberToString(_value.vec6f[3]) + ", ";
            result += numberToString(_value.vec6f[4]) + ", ";
            result += numberToString(_value.vec6f[5]) + "]";
            break;
      case ControlParameterValueKind::VEC6_DOUBLE:
            result += "[";
            result += numberToString(_value.vec6d[0]) + ", ";
            result += numberToString(_value.vec6d[1]) + ", ";
            result += numberToString(_value.vec6d[2]) + ", ";
            result += numberToString(_value.vec6d[3]) + ", ";
            result += numberToString(_value.vec6d[4]) + ", ";
            result += numberToString(_value.vec6d[5]) + "]";
            break;
      default:
  ...
  bool setFromString(const std::string& value) {
    switch (_kind) {
      ...
      case ControlParameterValueKind::VEC3_DOUBLE: {
        ...
        case ControlParameterValueKind::VEC6_FLOAT: {
            Vec6<float> v = stringToVec6<float>(value);
            _value.vec6f[0] = v[0];
            _value.vec6f[1] = v[1];
            _value.vec6f[2] = v[2];
            _value.vec6f[3] = v[3];
        ...
        case ControlParameterValueKind::VEC6_DOUBLE: {
            Vec6<double> v = stringToVec6<double>(value);
            _value.vec6d[0] = v[0];
            _value.vec6d[1] = v[1];
            _value.vec6d[2] = v[2];
            _value.vec6d[3] = v[3];
...
class ControlParameters {
 public:
  ...
  void initializeVec6d(const std::string& name, Vec6<double>& v) {
        collection.lookup(name).initializeVec6d(v);
    }

  void initializeVec6f(const std::string& name, Vec6<float>& v) {
        collection.lookup(name).initializeVec6f(v);
    }
  /*!
